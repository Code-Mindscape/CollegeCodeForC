    	...
    	if(userSelect == adultsTicket){
    		...
    		scanf("%d",&persons);
    			if(persons >= 5){
    				...
    				totalCostforComeBack = persons *10;
    				totalCost = persons*30 - discountRate;
    				printf("Total cost is: %.2f$\n",totalCost);
    				printf("Saved %.2f$\n",discountRate);
    				printf("If you come back we will offer 10$ for each ticket and the cost for %d tickets will be %.2f$.\n",persons,totalCostforComeBack);
